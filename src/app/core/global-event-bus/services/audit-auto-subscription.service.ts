...
export class AuditAutoSubscriptionService {
  ...
  private async handleEvent(event: DomainEvent): Promise<void> {
    try {
      ...
      await this.recordAudit(event, route.category, actor, resource, action, {
        level: route.level,
      ...
      console.error(`[AuditAutoSubscriptionService] Error handling event ${event.eventType}:`, error);
  ...
  private extractActor(event: DomainEvent): string {
    ...
    return payload?.userId || payload?.user?.id || payload?.actorId || payload?.actor?.id || 'system';
  }
  ...
  private extractResource(event: DomainEvent): string {
    const payload = event.payload as any;
    return event.aggregateType || payload?.resourceType || this.extractResourceFromEventType(event.eventType);
  }
  ...
  private async recordAudit(
    ...
    switch (category) {
      case AuditCategory.AUTHENTICATION:
        await this.auditCollector.recordAuth(event.eventId, event.eventType, actor, action, commonOptions);
        break;

      case AuditCategory.AUTHORIZATION:
        await this.auditCollector.recordAuthorization(event.eventId, event.eventType, actor, action, resource, resourceId, commonOptions);
        break;
      ...
      case AuditCategory.SECURITY:
        const description = (event.payload as any)?.description || (event.payload as any)?.threatType || event.eventType;
      ...
      case AuditCategory.COMPLIANCE:
        const complianceType = (event.payload as any)?.regulation || (event.payload as any)?.complianceType || 'unknown';
