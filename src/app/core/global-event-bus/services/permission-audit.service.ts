...
export class PermissionAuditService {
  ...
  readonly statistics = computed<PermissionAuditStatistics>(() => {
    ...
    return {
      ...
      permissionsGranted: events.filter(e => e.category === AuditCategory.PERMISSION && e.actionType === PermissionActionType.GRANT).length,
      permissionsRevoked: events.filter(e => e.category === AuditCategory.PERMISSION && e.actionType === PermissionActionType.REVOKE)
        .length,
      permissionsUpdated: events.filter(e => e.category === AuditCategory.PERMISSION && e.actionType === PermissionActionType.UPDATE)
        .length,
      rolesAssigned: events.filter(e => e.category === AuditCategory.ROLE && e.actionType === RoleActionType.ASSIGN).length,
      rolesUnassigned: events.filter(e => e.category === AuditCategory.ROLE && e.actionType === RoleActionType.UNASSIGN).length,
      rolesUpdated: events.filter(e => e.category === AuditCategory.ROLE && e.actionType === RoleActionType.UPDATE).length,
  ...
  readonly revokedPermissions = computed(() => {
    return this._auditEvents().filter(e => e.category === AuditCategory.PERMISSION && e.actionType === PermissionActionType.REVOKE);
  });

  /** 角色移除事件 (高風險) */
  readonly unassignedRoles = computed(() => {
    return this._auditEvents().filter(e => e.category === AuditCategory.ROLE && e.actionType === RoleActionType.UNASSIGN);
  });
  ...
  private subscribeToPermissionEvents(): void {
    // 訂閱權限變更事件
    this.eventBus
      .observe('auth.permission.changed')
  ...
  private detectAnomalousRoleChange(auditEvent: PermissionAuditEvent): boolean {
    ...
    const hasHighRiskRole = auditEvent.newState.roles?.some(r => highRiskRoles.some(risk => r.toLowerCase().includes(risk)));
   ...
   * 獲取資源的權限變更歷史
   */
  getResourcePermissionHistory(resourceType: string, resourceId: string, limit = 50): PermissionAuditEvent[] {
    return this._auditEvents()
