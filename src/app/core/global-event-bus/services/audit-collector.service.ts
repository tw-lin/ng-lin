...
import { AuditEvent, AuditEventBuilder, AuditLevel, AuditCategory, AuditChanges } from '../models/audit-event.model';
...
export class AuditCollectorService {
   ...
   */
  async collectFromDomainEvent(domainEvent: DomainEvent, options: AuditRecordOptions = {}): Promise<void> {
   ...
   */
  async recordAuth(eventId: string, eventType: string, actor: string, action: string, options: AuditRecordOptions = {}): Promise<void> {
    // Verify and extract tenant_id (MANDATORY)
   ...
   * 私有方法: 從領域事件創建審計事件
   */
  private createAuditEventFromDomain(domainEvent: DomainEvent, options: AuditRecordOptions, tenantId: string | null): AuditEvent {
    ...
    if (options.ipAddress || options.userAgent) {
      builder.withContext(options.ipAddress, options.userAgent, domainEvent.metadata?.correlationId);
    }
  ...
  private extractActor(event: DomainEvent): string {
    ...
    return payload?.userId || payload?.actorId || payload?.executedBy || (event.metadata as any)?.userId || 'system';
  }
