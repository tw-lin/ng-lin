...
export class VersionMigrationService {
   ...
   */
  deprecateVersion(eventType: string, version: string, migrateTo: string, message: string, removeAt?: Date): void {
   ...
   */
  async migrate<T extends DomainEvent<any>>(event: DomainEvent<any>, targetVersion: string): Promise<T> {
   ...
   */
  private trackMigration(eventType: string, fromVersion: string, toVersion: string, duration: number): void {
