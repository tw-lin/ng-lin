...
export class TenantValidationError extends Error {
  constructor(message: string, public readonly event: DomainEvent) {
    super(message);
...
export class TenantValidationMiddleware {
  ...
  private validateTenantId(tenantId: string, event: DomainEvent): void {
    ...
    if (!tenantId || tenantId.trim().length === 0) {
      throw new TenantValidationError(
        `Event ${event.eventType} rejected: tenant_id cannot be empty`,
        event
      );
    }
    ...
    if (/\s/.test(tenantId)) {
      throw new TenantValidationError(
        `Event ${event.eventType} rejected: tenant_id cannot contain whitespace`,
        event
      );
    }
