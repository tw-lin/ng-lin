 ...
 */
export abstract class BaseEventUpcaster<TFrom extends DomainEvent<any>, TTo extends DomainEvent<any>>
  implements EventUpcaster<TFrom, TTo> 
{
  ...
  upcast(event: TFrom): TTo {
    ...
    if (this.canUpcast && !this.canUpcast(event)) {
      throw new Error(
        `Cannot upcast event ${event.eventType} from ${this.fromVersion} to ${this.toVersion}`
      );
    }
  ...
  protected validateEvent(event: TFrom): void {
    if (event.eventType !== this.eventType) {
      throw new Error(
        `Event type mismatch: expected ${this.eventType}, got ${event.eventType}`
      );
    }
    ...
    if (event.metadata.version !== this.fromVersion) {
      throw new Error(
        `Version mismatch: expected ${this.fromVersion}, got ${event.metadata.version}`
      );
    }
  }
...
export class UpcasterRegistry {
   ...
   */
  getAll(eventType: string): EventUpcaster<any, any>[] {
    return Array.from(this.upcasters.values())
      .filter(u => u.eventType === eventType);
  }
