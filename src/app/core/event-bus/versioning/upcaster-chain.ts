...
export class UpcasterChain {
   ...
   */
  upcast<T extends DomainEvent<any>>(
    ...
    if (!path) {
      throw new Error(
        `No upcast path found for ${event.eventType} from ${currentVersion} to ${targetVersion}`
      );
    }
  ...
  private findUpcastPath(
    ...
    const queue: Array<{ version: string; path: UpcastStep[] }> = [
    ...
    while (queue.length > 0) {
      ...
      const upcasters = this.registry.getAll(eventType)
        .filter(u => u.fromVersion === current.version);
