...
@Component({
  ...
  template: `
    <div class="demo-container">
      ...
      <div class="metrics">
        <div class="metric">
          <strong>Total Events:</strong> {{ eventBus.totalEvents() }}
        </div>
        <div class="metric">
          <strong>Active Subscriptions:</strong> {{ eventBus.subscriptionCount() }}
        </div>
        <div class="metric">
          <strong>Tasks Created:</strong> {{ analyticsConsumer.totalTasksCreated() }}
        </div>
        <div class="metric">
          <strong>Tasks Completed:</strong> {{ analyticsConsumer.totalTasksCompleted() }}
        </div>
      </div>
      
      <div class="actions">
        <button (click)="createTask()">Create Task</button>
        <button (click)="updateTask()" [disabled]="recentTasks().length === 0">
          Update Last Task
        </button>
        <button (click)="completeTask()" [disabled]="recentTasks().length === 0">
          Complete Last Task
        </button>
      </div>
